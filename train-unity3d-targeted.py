...
class GoalPlayer(CloseablePlayer):
    def current_goal(self):
        return self.close_target.current_goal()
...
class Model(ModelDesc):
    def _get_inputs(self):
        ...
        return [InputDesc(tf.uint8, (None,) + IMAGE_SHAPE3, 'state'),
                ...
                InputDesc(tf.float32, (None,), 'goal')
                ]

    def _get_NN_prediction(self, image, goal):
        ...
        gl = FullyConnected('fc0-goal', goal, 512, nl=tf.identity)
        l = tf.multiply(l, gl)
        l = PReLU('prelu', l)
    ...
    def _build_graph(self, inputs):
        state, action, futurereward, action_prob, goal = inputs
        logits, value = self._get_NN_prediction(state, goal)
...
class MySimulatorMaster(SimulatorMaster, Callback):
    ...
    def _on_state(self, state_goal, ident):
        state = state_goal[0]
        goal = state_goal[1]
        def cb(outputs):
            try:
            ...
            client.memory.append(TransitionExperience(
                state, action, reward=None, value=value, prob=distrib[action], goal=goal))
            self.send_queue.put([ident, dumps(action)])
        self.async_predictor.put_task([state, goal], cb)
    ...
    def _parse_memory(self, init_r, ident, isOver):
        ...
        for idx, k in enumerate(mem):
            ...
            self.queue.put([k.state, k.action, R, k.prob, k.goal])
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            PeriodicTrigger(Evaluator(
                EVAL_EPISODE, ['state', 'goal'], ['policy'], get_eval_player),
                every_k_epochs=1),
...
if __name__ == '__main__':
    ...
    if args.task != 'train':
        ...
        if args.logdir:
            dirname = os.path.join('train_log', 'train-unity3d-goal-{}-{}'.format(ENV_NAME, args.logdir))
        else:
            dirname = os.path.join('train_log', 'train-unity3d-goal-{}'.format(ENV_NAME))
